            ...
            switch (parts[0]) {
                case "add":
                    try {
                        ...
                        for (int i = 0; i < labWorkData.length; i++) {
                            if (i==3) {
                                labWorkData[i] = ZonedDateTime.now().toString();
                                continue;
                            } else {
                        ...
                        Client.sendRequest(new Request("add", new LabWork(labWorkData), null, User.getInstance().getName(), User.getInstance().getPasswd()));
                ...
                case "updateId":
                    try {
                        ...
                        Client.sendRequest(new Request("updateId " + id, new LabWork(labWorkData), strId, User.getInstance().getName(), User.getInstance().getPasswd()));
                        labWorkData = new String[11];
                        isSpecialCommandExecuted = true;
                ...
                try {
                    if (line.contains("remove_by_id") || line.contains("remove_by_author") || line.contains("count_less_than_author")) {
                        if (parts.length < 2) {
                            ...
                            Client.sendRequest(new Request(line, null, parts[1], User.getInstance().getName(), User.getInstance().getPasswd()));
                        }
                    } else {
                        Client.sendRequest(new Request(line, null, null, User.getInstance().getName(), User.getInstance().getPasswd()));
                    }
